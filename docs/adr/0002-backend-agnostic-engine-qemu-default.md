...
The daemon talks to the engine through a narrow `trait Engine` (nine
methods: `create_session`, `destroy_session`, `set_enabled`, `set_param`,
`set_params`, `get_param`, `get_params`, `process`, `version`).
...
params in one call (it also feeds the visualizer pump, which reads the two
live `vcbg`/`vcbe` leaves), and `set_params` batches profile and EQ-preset
switches so the change applies on one audio block.
