...
1. **Intelligent EQ with `ieq_rich` at amount=10** — This isn't a static EQ. It _analyzes the content in real-time_ and shapes the spectrum to match the "rich" target curve. It boosts the midrange warmth (bands 5–11, ~650Hz–3kHz) while rolling off harsh treble. Amount=10 means it's at **maximum strength**.
