...
|   # | 4-CC   | len | bounds     | settable | Description                                                                                                                                                                                                                                   |
| --: | ------ | --: | ---------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|  55 | `endp` |   1 | 0..6       | no       | Output endpoint enum. Java's `DsEndpoint` order (SPEAKER, HEADPHONES, HDMI, SPDIF, DLNA, LINE_OUT, BLUETOOTH) is **not** the AK encoding: at the AK level **HEADPHONES = 2** — probe-verified (left-only crosstalk: the `vdhe = 2` auto gate engages only at `endp = 2`; `vdhe = 1` forces on at any endpoint), and v1's proven config wrote 2. Other slots unmapped. Writable via `ak_set` (lands + reads back); the service normally feeds it from the `setOutputDevice` HAL flow, not `setDsApParam`. |
