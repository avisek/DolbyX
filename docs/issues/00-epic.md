...
| **`EngineSupervisor`** (`ddp-daemon`) | `start()` · `shutdown()` · session ops mirroring `Engine`. Errors: `EngineCrashed`, `SessionInitFailed`, `SessionNotFound`. | Respawn on crash; creation-ordered session list (main = index 0); session init (INIT → SET_CONFIG at plugin rate → one `set_params` of resolved profile → ENABLE — no DEFINE_PARAMS/SETTINGS); readouts re-read on main change; vis fan-out; write fan-out to all sessions (none → state-only); created-while-off starts disabled. | 04, grown 08/11/16 |
